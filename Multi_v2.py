...
def worker_task(args):
    key_chunk, db_path, compare_cols, progress_queue = args
...
def run_compare():
    ...
    try:
        ...
        chunks = [keys[i::cpu_count] for i in range(cpu_count)]

        manager = mp.Manager()
        progress_queue = manager.Queue()

        args = [(chunk, db_path, compare_cols, progress_queue) for chunk in chunks]
        ...
        def track_progress():
            ...
            try:
                while True:
                    ...
                    eta = (total_keys - processed) / rate if rate else 0

                    progress_bar["maximum"] = total_keys
                    progress_bar["value"] = processed

                    update_status(
                        f"{processed}/{total_keys} | "
        ...
        with open(output_file, "w", newline="", encoding="utf-8") as out_f:
            ...
            for temp_path, part_summary in results:
                ...
                for col in compare_cols:
                    summary[col]["total"] += part_summary[col]["total"]
                    summary[col]["mismatch"] += part_summary[col]["mismatch"]
